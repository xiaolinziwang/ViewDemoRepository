...
public abstract class BarLineChartBase<T extends BarLineScatterCandleBubbleData<? extends IBarLineScatterCandleBubbleDataSet<? extends Entry>>> extends Chart<T> implements BarLineScatterCandleBubbleDataProvider {
    ...
    protected void init() {
        ...
        mChartTouchListener = new BarLineChartTouchListener(this, mViewPortHandler
                .getMatrixTouch());
    ...
    protected void prepareValuePxMatrix() {
        ...
        mRightAxisTransformer
                .prepareMatrixValuePx(mXAxis.mAxisMinimum, mXAxis.mAxisRange, mAxisRight.mAxisRange, mAxisRight.mAxisMinimum);
        mLeftAxisTransformer
                .prepareMatrixValuePx(mXAxis.mAxisMinimum, mXAxis.mAxisRange, mAxisLeft.mAxisRange, mAxisLeft.mAxisMinimum);
    }
    ...
    protected void calcMinMax() {
        ...
        mAxisRight.calcMinMax(mData.getYMin(AxisDependency.RIGHT), mData
                .getYMax(AxisDependency.RIGHT));
    }
    ...
    public void calculateOffsets() {
        ...
        if (!mCustomViewPortEnabled) {
            ...
            mViewPortHandler.restrainViewPort(Math.max(minOffset, offsetLeft), Math
                    .max(minOffset, offsetTop), Math.max(minOffset, offsetRight), Math
    ...
    public boolean onTouchEvent(MotionEvent event) {
        ...
        if (mMarkerView != null && MotionEvent.ACTION_UP == event.getAction() &&
                mMarkerView.isTouch(event)) {
            mMarkerView.markViewClick();
            return true;
        }
     ...
     * x and y are the values (NOT PIXELS) which to zoom to or from (the values of the zoom
     * center).
     *
    ...
    public void zoomAndCenterAnimated(float scaleX, float scaleY, float xValue, float yValue, AxisDependency axis, long duration) {
        ...
        if (android.os.Build.VERSION.SDK_INT >= 11) {
            ...
            PointD origin = getValuesByTouchPoint(mViewPortHandler.contentLeft(), mViewPortHandler
                    .contentTop(), axis);

            Runnable job = new AnimatedZoomJob(mViewPortHandler, this, getTransformer(axis), getAxis(axis), mXAxis
                    .getValues().size(), scaleX, scaleY, mViewPortHandler
                    .getScaleX(), mViewPortHandler
                    .getScaleY(), xValue, yValue, (float) origin.x, (float) origin.y, duration);
            addViewportJob(job);
        } else {
    ...
    public void moveViewToY(float yValue, AxisDependency axis) {
        ...
        Runnable job = new MoveViewJob(mViewPortHandler, 0f,
    ...
    public void moveViewTo(float xIndex, float yValue, AxisDependency axis) {
        ...
        Runnable job = new MoveViewJob(mViewPortHandler, xIndex,
                yValue + valsInView / 2f, getTransformer(axis), this);
    ...
    public void moveViewToAnimated(float xIndex, float yValue, AxisDependency axis, long duration) {
        ...
        if (android.os.Build.VERSION.SDK_INT >= 11) {
            ...
            PointD bounds = getValuesByTouchPoint(mViewPortHandler.contentLeft(), mViewPortHandler
                    .contentTop(), axis);

            float valsInView = getDeltaY(axis) / mViewPortHandler.getScaleY();

            Runnable job = new AnimatedMoveViewJob(mViewPortHandler, xIndex, yValue + valsInView /
                    2f, getTransformer(axis), this, (float) bounds.x, (float) bounds.y, duration);
    ...
    public void centerViewTo(float xIndex, float yValue, AxisDependency axis) {
        ...
        Runnable job = new MoveViewJob(mViewPortHandler,
                xIndex - xsInView / 2f, yValue + valsInView / 2f, getTransformer(axis), this);
    ...
    public void centerViewToAnimated(float xIndex, float yValue, AxisDependency axis, long duration) {
        ...
        if (android.os.Build.VERSION.SDK_INT >= 11) {
            ...
            Runnable job = new AnimatedMoveViewJob(mViewPortHandler,
                    xIndex - xsInView / 2f, yValue + valsInView /
                    2f, getTransformer(axis), this, (float) bounds.x, (float) bounds.y, duration);
